...
enum UiEventType : uint8_t { UIE_KEY = 1, UIE_ROTARY = 2, UIE_BATT = 3 };
struct UiEvent {
    UiEventType type;
...
// --------------- I2C Bus Mutex for Wire_main_mcu (seesaw + main MCU) --------
...
void setup(void)
{
    ...
    // Create mutex for I2C bus 1 shared by encoder + main MCU
    ...
    // Bind seesaw to the dedicated Wire_main_mcu bus to avoid contention with LVGL/touch on default Wire
    if (!ss.begin(SEESAW_ADDR, -1, &Wire_main_mcu))
    {
...
void encoderTask(void *parameter)
{
    ...
    while (1)
    {
        if (millis() > checkMs)
        {
            if (i2c1Mutex && xSemaphoreTake(i2c1Mutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            if (!ss.digitalRead(SS_SWITCH_UP))
            {
                Serial.println("UP pressed!");
                if (uiEventQueue) { UiEvent ev{UIE_KEY, LV_KEY_UP, 0}; xQueueSend(uiEventQueue, &ev, 0); }
                checkMs = millis() + debounceTime;
            }
            if (!ss.digitalRead(SS_SWITCH_DOWN))
            {
                Serial.println("DOWN pressed!");
                if (uiEventQueue) { UiEvent ev{UIE_KEY, LV_KEY_DOWN, 0}; xQueueSend(uiEventQueue, &ev, 0); }
                checkMs = millis() + debounceTime;
            }
            if (!ss.digitalRead(SS_SWITCH_LEFT))
            {
                Serial.println("LEFT pressed!");
                if (uiEventQueue) { UiEvent ev{UIE_KEY, LV_KEY_LEFT, 0}; xQueueSend(uiEventQueue, &ev, 0); }
                checkMs = millis() + debounceTime;
            }
            if (!ss.digitalRead(SS_SWITCH_RIGHT))
            {
                Serial.println("RIGHT pressed!");
                if (uiEventQueue) { UiEvent ev{UIE_KEY, LV_KEY_RIGHT, 0}; xQueueSend(uiEventQueue, &ev, 0); }
                checkMs = millis() + debounceTime;
            }
            if (!ss.digitalRead(SS_SWITCH_SELECT))
            {
                Serial.println("SELECT pressed!");
                if (uiEventQueue) { UiEvent ev{UIE_KEY, LV_KEY_ENTER, 0}; xQueueSend(uiEventQueue, &ev, 0); }
                checkMs = millis() + debounceTime;
            }
        ...
        if (i2c1Mutex && xSemaphoreTake(i2c1Mutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            ...
            // If mutex timed out, skip this iteration to avoid bus contention
        ...
        if (encoder_diff_position != new_diff_position)
        {
            ...
            // Route rotary change to UI thread
            if (uiEventQueue) { UiEvent ev{UIE_ROTARY, (uint32_t)encoder_position, 0}; xQueueSend(uiEventQueue, &ev, 0); }
        }

        vTaskDelay(5 / portTICK_PERIOD_MS); // modest polling to reduce I2C bus pressure
    }
}
...
void readBatteryLifeTask(void *parameter)
{
    for (;;) // infinite loop
    {
        ...
        if (Wire_main_mcu.available() == DATA_LENGTH)
        {
            ...
            // Log & send voltage update to UI thread
