...
{
    Serial.begin(115200);
    // Serial is the native USB-CDC: with no host draining it (monitor
    // closed / cable pulled), writes otherwise BLOCK until a timeout —
    // which stalls every printing task, including the 500 Hz motor loop.
    // Zero timeout = drop instantly when nobody is listening.
