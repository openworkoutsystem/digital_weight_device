...
def rgb565(r, g, b):
    ...
    b = (b >> 3) & 0x1F
    v = (r << 11) | (g << 5) | b
    # Panel expects big-endian pixel data; pre-swap bytes in the stored array.
    return ((v & 0xFF) << 8) | (v >> 8)
...
def main(png_path, output_path):
    ...
    # Target canvas: panel-native portrait coordinates, full screen 240x536.
    # pushColors() writes directly in panel coords (240 wide x 536 tall);
    # the image is pre-rotated above so it reads landscape on the device.
    display_width = 240
    display_height = 536
