...
void beginLvglHelper(LilyGo_Display &board, bool debug)
{
    ...
    size_t pixel_count = (size_t)board.width() * (size_t)board.height();
    size_t lv_buffer_bytes = pixel_count * sizeof(lv_color_t);
    buf = (lv_color_t *)ps_malloc(lv_buffer_bytes);
    assert(buf);
    ...
    // The last parameter expects number of pixels, not bytes
