...
public:
    ...
    // Brightness used while showing the boot splash (ramp up in the app after
    // boot). Dim boot level reduces the AMOLED inrush current on the 5V rail.
    void setBootBrightness(uint8_t level);

    // True once the deferred init-sequence replay has completed.
    bool finalizeDone();
    // Redraw the boot splash. Call from the LVGL/main task only (the SPI path
    // has no lock), after finalizeDone() — e.g. to recover the splash on cold
    // power-ups where the panel was not yet ready for the first draw.
    void redrawSplash();
...
private:
    ...
    volatile bool _finalized = false;
    ...
    struct BootOptions {
        ...
        // Splash brightness (0-255); kept low so the panel doesn't step to
        // full drive current at the most brownout-prone moment of boot
        uint8_t bootBrightness = 40;
